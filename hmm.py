...
def filter( stateMap, stateIndex, obsMap, obsIndex, prob, tprob, eprob, observations):
    ...
    """
    pdist = []
    prevDist = weatherProb
    for i in range(0, len(stateMap)):
		pdist.append(0)
    for obs in observations:
    #for t in observations
    #	step1: elapse time: marginal belief distribution over current state
    #	step2: update belief (sum of all posible states transformation)
    #	step3: normalize belief distribution;
    	pdis = []
    	for i in range(0, len(stateMap)):
			pdist.append(0)
		...
		for curState in stateMap:
			...
			for prevState in stateMap:
				...
				pdist[i] = pdist[i] + prevDist[j] * getNextStateProb(weatherProb,weatherStateMap,prevState,curState)

			for curState in stateMap:
				i = stateMap[curState]
				j = stateMap[prevState]
				pdist[i] = pdist[i] +getObservationProb(weatherEProb,stateMap,obsMap,curState,obs)
		pdist = normalize(pdist)
		prevDist=pdist #priors
...
def predict(stateMap, stateIndex, obsMap, obsIndex, prob, tprob, eprob, observations):
    ...
    for curState in stateMap:
    	...
    	for prevState in stateMap:
    		j = stateMap[prevState]
    		nextDist[i] = nextDist[i] + curState[j] * getNextStateProb(weatherTProb, stateMap, prevState, curState)
